	...
	social_worker: {
		...
		policies: [
			...
			},
			{
				resource: "clinical_note",
				action: ["read", "write"],
				conditions: [{ attribute: "noteType", operator: "in", value: ["psychosocial", "assessment"] }],
				effect: "allow",
	...
	speech_therapist: {
		...
		policies: [
			...
			},
			{
				resource: "clinical_note",
				action: ["read", "write"],
				conditions: [{ attribute: "noteType", operator: "in", value: ["slp_eval", "slp_treatment"] }],
				effect: "allow",
		...
		policies: [
			{
				resource: "patient",
				action: ["read", "write"],
				conditions: [{ attribute: "status", operator: "in", value: ["referral", "pending", "admitted"] }],
				effect: "allow",
		...
		policies: [
			{
				resource: "patient",
				action: "read",
				conditions: [{ attribute: "fields", operator: "in", value: ["name", "mrn", "schedule_needs"] }],
				effect: "allow",
		...
		policies: [
			{
				resource: "patient",
				action: "read",
				conditions: [{ attribute: "fields", operator: "in", value: ["name", "medications", "allergies"] }],
				effect: "allow",
			...
			},
			{
				resource: "medication",
				action: ["read", "write"],
				conditions: [{ attribute: "action", operator: "in", value: ["review", "interaction_check"] }],
				effect: "allow",
		...
		policies: [
			{
				resource: "patient",
				action: "read",
				conditions: [{ attribute: "fields", operator: "in", value: ["name", "address", "dme_needs"] }],
				effect: "allow",
		...
		policies: [
			{
				resource: "patient",
				action: "read",
				conditions: [{ attribute: "surveyType", operator: "in", value: ["complaint", "recertification", "annual"] }],
				effect: "allow",
...
export class ABACService {
	 ...
	 */
	static isInGroup(role: UserRole, group: keyof typeof RoleGroups): boolean {
	...
	/**
	 * Check if action is allowed for role on resource
	 */
	static can(
		role: UserRole,
		action: ABACAction,
	 ...
	 */
	static getAllowedActions(role: UserRole, resource: ABACResource): ABACAction[] {
		const actions: ABACAction[] = ["read", "write", "delete", "sign", "export", "admin"];
		return actions.filter((action) => this.can(role, action, resource));
	}
	...
	private static evaluateCondition(
		...
		switch (condition.operator) {
			...
			case "contains":
				return (
					Array.isArray(value) && Array.isArray(condition.value) &&
	...
	/**
	 * Require role group membership
	 */
	static requireGroup(group: keyof typeof RoleGroups) {
		return (role: UserRole): boolean => {
			return this.isInGroup(role, group);
		};
	}
}

// Export convenience functions
export const can = ABACService.can.bind(ABACService);
export const isInGroup = ABACService.isInGroup.bind(ABACService);
export const getRoleDefinition = ABACService.getRoleDefinition.bind(ABACService);
export const getAllowedActions = ABACService.getAllowedActions.bind(ABACService);
