...
export async function fetchPatientF2F(
	...
	return apiFetch<F2FEncounterListResponse>(
...
export async function validateF2F(
	id: string,
	cookieHeader: string,
): Promise<F2FValidityResult> {
	return apiFetch<F2FValidityResult>(
		`/api/v1/f2f/${id}/validate`,
		cookieHeader,
		{ method: "POST" },
	);
}
...
export const createF2FFn = createServerFn({ method: "POST" })
	.inputValidator(
		(data: unknown) => data as { patientId: string; body: CreateF2FInput },
	)
	.handler(async ({ data }) => {
		const request = getRequest();
		const cookie = request.headers.get("cookie") ?? "";
