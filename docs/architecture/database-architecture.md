...
| **Analytics** | 17 | 5% | HOPE assessments (replaces HIS), HQRP quality measures, CAHPS, HCI |
...
### Analytics Domain — HOPE Quality Reporting (17 schemas)

> **Note:** HIS (Hospice Item Set) was deprecated by CMS effective October 1, 2025.
> All quality reporting now uses **HOPE (Hospice Outcomes and Patient Evaluation)** submitted via iQIES.
> See `docs/compliance/hope-reporting.md` for the complete specification.

#### HOPE Assessments (9 schemas)

| Schema | TypeBox Definition | Purpose |
|--------|-------------------|---------|
| `hope.schema.ts` | `HOPEAdmissionSchema` | HOPE-A: full assessment within 7 days of election |
| `hope.schema.ts` | `HOPEUpdateVisitSchema` | HOPE-UV: per-visit abbreviated assessment |
| `hope.schema.ts` | `HOPEDischargeAssessmentSchema` | HOPE-D: within 7 days of discharge/death |
| `hope.schema.ts` | `HOPEiQIESSubmissionSchema` | iQIES submission tracking (retries, DLQ) |
| `hope.schema.ts` | `HOPEAdministrativeSchema` | Section A: CMS CCN, patient demographics |
| `hope.schema.ts` | `HOPECognitiveSchema` | Section C: BIMS + CAM delirium screening |
| `hope.schema.ts` | `HOPEMoodSchema` | Section D: PHQ-2 depression screening |
| `hope.schema.ts` | `HOPEFunctionalStatusSchema` | Section F: ADL self-performance (9 domains) |
| `hope.schema.ts` | `HOPEPainSchema` | Section J: multi-modal pain (NRS + verbal + behavioral) |

#### HQRP Quality Measures (5 schemas)

| Schema | TypeBox Definition | NQF # | Description |
|--------|-------------------|-------|-------------|
| `hopeQualityMeasures.schema.ts` | `HOPEComprehensiveAssessmentMeasureSchema` | #3235 | Comprehensive assessment at admission (7 domains) |
| `hopeQualityMeasures.schema.ts` | `HOPETreatmentPreferencesMeasureSchema` | #3633 | Treatment preferences documented |
| `hopeQualityMeasures.schema.ts` | `HOPEHVLDLMeasureSchema` | #3634 | Visits when death is imminent (RN/MD + SWW/chaplain) |
| `hopeQualityMeasures.schema.ts` | `HOPEHospiceCareIndexSchema` | HCI | 10-indicator composite score |
| `hopeQualityMeasures.schema.ts` | `HOPEReportingPeriodSchema` | — | HQRP quarterly reporting period tracker |

#### HOPE Database Tables

| Table | Key Columns (native) | JSONB | RLS Tier |
|-------|---------------------|-------|----------|
| `hope_assessments` | `patient_id`, `location_id`, `assessment_type`, `assessment_date`, `election_date`, `status` | `data` (full HOPE payload) | Tier 2 (clinical) |
| `hope_iqies_submissions` | `assessment_id`, `location_id`, `response_status`, `submitted_at` | `response_errors` | Tier 2 |
| `hope_reporting_periods` | `location_id`, `calendar_year`, `quarter`, `submission_status`, `penalty_applied` | — | Tier 2 |
| `hope_quality_measures` | `location_id`, `period_start`, `period_end`, `measure_code` | `measure_data` | Tier 2 |
