...
function AlertCard({
	...
	alert: Alert;
	onStatusChange: (id: string, status: Alert["status"], extra?: { snoozedUntil?: string; assignedTo?: string }) => void;
}) {
		...
		>
			<div className="flex items-start justify-between gap-4">
				<div className="flex items-start gap-3 flex-1 min-w-0">
					...
					<span className={`text-2xl flex-shrink-0 ${isHardBlock && alert.status === "new" ? "animate-pulse" : ""}`}>
						{typeIcon(alert.type)}
					</span>
					...
					<div className="flex-1 min-w-0">
						...
						<div className="flex items-center gap-2 flex-wrap mb-1">
							<span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${severityBadge(alert.severity)}`}>
...
function AlertsDashboard() {
	...
	const patchMutation = useMutation({
		mutationFn: ({ alertId, body }: { alertId: string; body: import("@hospici/shared-types").AlertStatusPatchBody }) =>
	...
	const criticalCount = operationalAlerts.filter((a) => a.severity === "critical" && a.status !== "resolved").length;
...
// biome-ignore lint/correctness/noUnusedVariables: used in AlertsDashboard
function useAlertCount(_alerts: AlertListResponse | undefined) {
	void _alerts;
}
