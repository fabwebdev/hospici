...
import {
  ...
  AlertService,
  AlertSnoozeError,
} from "@/contexts/compliance/services/alert.service.js";
...
export default async function alertRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.patch(
          ...
          if (!Validators.AlertStatusPatchBody.Check(request.body)) {
            return reply.code(400).send({
              error: {
                ...
                errors: [...Validators.AlertStatusPatchBody.Errors(request.body)].map((e) => ({
                  path: e.path,
                  message: e.message,
                })),
              },
    ...
    async (request, reply) => {
      ...
      try {
        let alert: Awaited<ReturnType<typeof alertService.acknowledgeAlert>>;
        ...
        if (err instanceof AlertSnoozeError) {
          return reply
            .code(422)
            .send({ error: { message: err.message, code: "HARD_BLOCK_NO_SNOOZE" } });
        }
