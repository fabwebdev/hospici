...
import { HARD_BLOCK_ALERT_TYPES, type AlertType, type UpsertAlertInput } from "@hospici/shared-types";
	...
	async listAlerts(
		...
		const rows = await db.transaction(async (tx) => {
			...
			if (filters.assignedTo) {
				query = query.where(
					eq(complianceAlerts.assignedTo, filters.assignedTo),
				) as typeof query;
			}
			if (filters.severity) {
				query = query.where(
					eq(complianceAlerts.severity, filters.severity),
				) as typeof query;
			}
